...
- Keep `Driver` and `Diagnostics` free of virtual functions. `Diagnostics::render` is a `std::function` member, not an override, for that reason: a polymorphic `Driver` has a vtable, and a vtable exported from a shared library is a *data* symbol that Windows only resolves through `__declspec(dllimport)` - without it the linker silently binds it to a call thunk and the first virtual dispatch jumps into hyperspace.
