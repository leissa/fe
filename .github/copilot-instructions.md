...
- `fe::Sets` (`sets.h`) hash-conses sets of pointers - small ones as sorted arrays, large ones as paths in a trie built on the intrusive link-cut-tree of `link_cut_tree.h` - so that equal sets are pointer-equal.
