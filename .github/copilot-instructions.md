...
- `fe::Patricia` (`patricia.h`) hash-conses immutable sets of pointers: a *key* trait passed as template argument reads the unsigned id an element carries, and a `Set` iterates `D*` in ascending id order.
...
Support headers: `algo.h` (bit casts, padding, small string/range algorithms), `assert.h` (`assert`/`assertf`/`unreachable`), `cast.h` (checked/dynamic casts), `cli.h` (`fe::Cli`, a single-command `argc`/`argv` parser that renders its help for a terminal or as Markdown tables), `container.h` (`pop`/`lookup` helpers, `Stacklike`/`Queuelike` concepts, `StrMap`/`StrSet` with `std::string_view` lookup), `dbg.h` (`fe::Dbg`, a `Loc`/`Sym` pair), `enum.h` (bit-flag enum ops), `format.h` (`ostream_formatter`, `std::format` glue), `hash.h` (`constexpr` hash mixing/combining), `log.h` (`fe::Log`, leveled logging; its `e`/`w`/... shorthands capture the call site with `std::source_location` and take a `cite_string` like `fe::Error`), `restore.h` (`fe::Restore`, an RAII guard that restores a reference - or a getter/setter pair - at end of scope), `span.h` (`fe::Span`/`fe::View`), `term.h` (terminal/ANSI color, incl. `fe::term::ScopedMode`; also `fe::throwf`, which lives next to the markup it renders), `utf8.h` (UTF-8 decode primitives plus the ASCII ctype set below), `vector.h` (`fe::Vector`, small-buffer vector over `ankerl::svector`).
...
- `fe::throwf` takes a `cite_string` and renders it *at the throw*, so its `what()` is finished text like `Error::Bail`'s - never markup a generic `catch` would print raw. It lives in `term.h` rather than `assert.h` because the latter is included by `term.h`.
